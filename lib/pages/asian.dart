...
class Asian extends StatefulWidget {
  ...
  const Asian({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _AsianState extends State<Asian> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          asianFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: asianFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: asianFoodData!.length,
                itemBuilder: (context, index) {
                  final item = asianFoodData![index];
                  return ListTile(
