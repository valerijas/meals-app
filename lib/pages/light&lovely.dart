...
class LightLovely extends StatefulWidget {
  final FavoriteRecipes favoriteRecipes;

  const LightLovely({Key? key, required this.favoriteRecipes})
      : super(key: key);

  @override
  _LightLovelyState createState() => _LightLovelyState();
}

class _LightLovelyState extends State<LightLovely> {
  ...
  final logger = Logger();
  List<dynamic>? LightLovelyFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          LightLovelyFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Light&Lovely Food'),
        ),
        body: LightLovelyFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: LightLovelyFoodData!.length,
                itemBuilder: (context, index) {
                  final item = LightLovelyFoodData![index];
                  return ListTile(
