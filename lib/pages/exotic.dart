...
class Exotic extends StatefulWidget {
  ...
  const Exotic({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _ExoticState extends State<Exotic> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          ExoticFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Exotic Food'),
        ),
        body: ExoticFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: ExoticFoodData!.length,
                itemBuilder: (context, index) {
                  final item = ExoticFoodData![index];
                  return ListTile(
