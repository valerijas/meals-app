...
class German extends StatefulWidget {
  ...
  const German({Key? key, required this.favoriteRecipes}) : super(key: key);

  @override
  _GermanState createState() => _GermanState();
}
...
class _GermanState extends State<German> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          GermanFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('German Food'),
        ),
        body: GermanFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: GermanFoodData!.length,
                itemBuilder: (context, index) {
                  final item = GermanFoodData![index];
                  return ListTile(
