...
class Summer extends StatefulWidget {
  ...
  const Summer({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _SummerState extends State<Summer> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          SummerFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Summer Food'),
        ),
        body: SummerFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: SummerFoodData!.length,
                itemBuilder: (context, index) {
                  final item = SummerFoodData![index];
                  return ListTile(
