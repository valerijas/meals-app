...
class Vegan extends StatefulWidget {
  ...
  const Vegan({Key? key, required this.favoriteRecipes}) : super(key: key);

  @override
  _VeganState createState() => _VeganState();
}
...
class _VeganState extends State<Vegan> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          VeganFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: VeganFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: VeganFoodData!.length,
                itemBuilder: (context, index) {
                  final item = VeganFoodData![index];
                  return ListTile(
