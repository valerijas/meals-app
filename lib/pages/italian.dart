...
class Italian extends StatefulWidget {
  ...
  const Italian({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _ItalianState extends State<Italian> {
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          ItalianFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ItalianFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: ItalianFoodData!.length,
                itemBuilder: (context, index) {
                  final item = ItalianFoodData![index];
                  return ListTile(
