...
class Vegetarian extends StatefulWidget {
  ...
  const Vegetarian({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _VegetarianState extends State<Vegetarian> {
  ...
  final logger = Logger();
  List<dynamic>? VegetarianFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          VegetarianFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: VegetarianFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: VegetarianFoodData!.length,
                itemBuilder: (context, index) {
                  final item = VegetarianFoodData![index];
                  return ListTile(
