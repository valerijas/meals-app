...
class Christmas extends StatefulWidget {
  ...
  const Christmas({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _ChristmasState extends State<Christmas> {
  ...
  final logger = Logger();
  List<dynamic>? ChristmasFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          ChristmasFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Christmas Food'),
        ),
        body: ChristmasFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: ChristmasFoodData!.length,
                itemBuilder: (context, index) {
                  final item = ChristmasFoodData![index];
                  return ListTile(
