...
class QuickEasy extends StatefulWidget {
  ...
  const QuickEasy({Key? key, required this.favoriteRecipes}) : super(key: key);

  @override
  _QuickEasyState createState() => _QuickEasyState();
}
...
class _QuickEasyState extends State<QuickEasy> {
  ...
  final logger = Logger();
  List<dynamic>? QuickEasyFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          QuickEasyFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Quick&Easy Food'),
        ),
        body: QuickEasyFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: QuickEasyFoodData!.length,
                itemBuilder: (context, index) {
                  final item = QuickEasyFoodData![index];
                  return ListTile(
