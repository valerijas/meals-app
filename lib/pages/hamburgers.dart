...
class Hamburgers extends StatefulWidget {
  ...
  const Hamburgers({Key? key, required this.favoriteRecipes}) : super(key: key);

  @override
  _HamburgersState createState() => _HamburgersState();
}
...
class _HamburgersState extends State<Hamburgers> {
  ...
  final logger = Logger();
  List<dynamic>? HamburgersFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          HamburgersFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: HamburgersFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: HamburgersFoodData!.length,
                itemBuilder: (context, index) {
                  final item = HamburgersFoodData![index];
                  return ListTile(
