...
class Breakfast extends StatefulWidget {
  ...
  const Breakfast({Key? key, required this.favoriteRecipes}) : super(key: key);
...
class _BreakfastState extends State<Breakfast> {
  ...
  final logger = Logger();
  List<dynamic>? BreakfastFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          BreakfastFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: BreakfastFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: BreakfastFoodData!.length,
                itemBuilder: (context, index) {
                  final item = BreakfastFoodData![index];
                  return ListTile(
