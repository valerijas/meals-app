...
class GlutenFree extends StatefulWidget {
  final FavoriteRecipes favoriteRecipes;

  const GlutenFree({Key? key, required this.favoriteRecipes}) : super(key: key);

  @override
  _GlutenFreeState createState() => _GlutenFreeState();
}

class _GlutenFreeState extends State<GlutenFree> {
  ...
  final logger = Logger();
  List<dynamic>? GlutenFreeFoodData;
  ...
  Future<void> fetchApiData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        logger.d(data);
        setState(() {
          GlutenFreeFoodData = data['results'] as List<dynamic>?;
        });
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: Colors.black,
          title: const Text('Gluten Free Food'),
        ),
        body: GlutenFreeFoodData == null
            ? const Center(
                child: CircularProgressIndicator(),
              )
            : ListView.builder(
                itemCount: GlutenFreeFoodData!.length,
                itemBuilder: (context, index) {
                  final item = GlutenFreeFoodData![index];
                  return ListTile(
